...
if (-not (Test-Path "deploy-config.ps1")) {
    ...
    Write-Host "Please copy scripts/deploy-config.template.ps1 to deploy-config.ps1 and configure your AWS settings." -ForegroundColor Yellow
    Write-Host "Run: Copy-Item scripts/deploy-config.template.ps1 deploy-config.ps1" -ForegroundColor Blue
    exit 1
}
